...
async function fetchProfile(userId: string): Promise<User | null> {
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select("id, name, email, location, admin_key, auto_publish, is_verified")
    .eq("id", userId)
    .single();
  ...
  return {
    ...
    isVerified: data.is_verified ?? undefined,
  };
}

function buildProfilePayload(
  authUser: SupabaseUser,
  fallback?: { name?: string; location?: string; adminKey?: string }
) {
  ...
  return {
    ...
    location:
      (meta.location as string) || fallback?.location || "Lagos, Nigeria",
