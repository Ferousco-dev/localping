...
export async function getUsers() {
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select("id, name, email, auto_publish, is_verified")
  ...
  return data.map((row) => ({
    ...
    Pick<User, "id" | "name" | "email" | "autoPublish" | "isVerified">
  >;
}

export async function updateUserAutoPublish(
  userId: string,
  autoPublish: boolean
) {
...
export async function rejectNews(id: string, adminId: string) {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from("news")
    .update({
...
export async function addNews(
  news: Omit<NewsItem, "id" | "date"> & { date?: string }
) {
  if (!supabase) return null;
  const newsType = news.newsType ?? "update";
  const payload = {
    title: news.title,
    ...
    news_type: newsType,
    community_kind:
      news.communityKind ?? (newsType === "update" ? "update" : "post"),
    status: "approved",
    verified: true,
...
export async function verifyUser(userId: string) {
  ...
  } as Pick<User, "id" | "name" | "email" | "autoPublish" | "isVerified">;
}
